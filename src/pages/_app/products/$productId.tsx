...
export const Route = createFileRoute("/_app/products/$productId")({
  ...
  head: ({ params }) => {
    ...
    const title = productFound
      ? `SyntaxWear - ${productFound.name}`
      : "SyntaxWear - Produto não encontrado";
...
function RouteComponent() {
  ...
  const originalPrice = productFound?.price ?? 0;
  ...
  return (
    <section className="container pt-44 mb-10 md:px-10 lg:min-h-[80vh]">
      ...
      <div className="flex flex-col lg:flex-row items-center justify-center gap-10">
        ...
        <div>
          <h1 className="text-4xl font-bold mb-1">{productFound?.name}</h1>

          <p className="mb-2">Cor: {productFound?.color}</p>
          ...
          <p className="mb-2">
            ...
            <span className="text-[#38373A] font-semibold">
              {formatCurrency(originalPrice / 6)} sem juros
            </span>
          </p>

          <p className="max-w-125 my-5">{productFound?.description}</p>
